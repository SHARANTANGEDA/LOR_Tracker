...
class ProjectsDone extends Component {
	...
	componentDidMount() {
		if(this.props.data.length!==0) {
			console.log("HERE", this.props.data, this.props.data.length)
			this.setState({projects: this.props.data, projectCnt: this.props.data.length+1})
			let years=[], projectTitles = [];
			for(let i=0; i<=this.props.data.length; i++) {
				projectTitles.push('project-' + i);
				years.push('year-' + i);
			}
			this.setState({years: years, projectTitles: projectTitles})
		}
	}
	...
	onRemove(index) {
		...
		this.setState({projects: [{
			...
			years: ['year-0'],
			projectTitles: ['project-0'],});
		this.props.checkbox.editData.projects_done=this.state.projects;
		}else {
