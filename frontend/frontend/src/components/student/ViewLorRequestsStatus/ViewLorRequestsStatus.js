	...
	render() {
		...
		const editButton = row => {
			...
			if(status==='AP' || status==='AC') {
				...
				return <span style={{background: 'grey', color: 'white', borderRadius: '5px', fontSize:'18px'}}>
		...
		const deleteButton = row => {
			...
			if(status==='AP' || status==='AC')
			{
				...
				return <button style={{background: 'grey', color: 'white', borderRadius: '5px', fontSize: '18px'}}>
					Cannot Withdraw now</button>
			}
		};
		...
		return (
			<div className='container-fluid' style={{minWidth: '100%', padding: '0px'}}>
				<div className="displayFolder ">
					<h3 className='text-center text-capitalize'>View Applied LOR Requests</h3>
					{heading}
