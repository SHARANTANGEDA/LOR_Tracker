...
} from "../../../../actions/lorActions";
import SearchBar from "../../../dashboard/SearchBar";
import AcceptedLorItem from "./AcceptedLorItem";


class ViewAllAcceptedRequests extends Component {
	constructor() {
		super();
		this.state = {
			currentPage: 1,
	...
	render() {
		...
		if (acceptedLoading || acceptedRequests === null) {
			...
			if (acceptedRequests.length === 0) {
				...
				const currentFolder = acceptedRequests.slice(indexOfFirstTodo, indexOfLastTodo);
				const render = (currentFolder.map(lor => (
					// <ProductCard folder={land} key={land._id}/>
					<AcceptedLorItem lorItem={lor} key={currentFolder.indexOf(lor)}/>
				)));
				for (let i = 1; i <= Math.ceil(acceptedRequests.length / todosPerPage); i++) {
					pageNumbers.push(i);
				}
...
ViewAllAcceptedRequests.propTypes = {
...
export default connect(mapStateToProps, {getAllAcceptedRequests})(ViewAllAcceptedRequests)
