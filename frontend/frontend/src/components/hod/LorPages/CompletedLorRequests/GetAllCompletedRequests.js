...
class GetAllCompletedRequests extends Component {
	constructor() {
		super();
		this.state = {
			currentPage: 1,
	...
	render() {
		...
		if (completedLoading || completedRequests === null) {
			...
			if (completedRequests.length === 0) {
				...
				const currentFolder = completedRequests.slice(indexOfFirstTodo, indexOfLastTodo);
				const render = (currentFolder.map(lor => (
					// <ProductCard folder={land} key={land._id}/>
					<CompletedLorItem lorItem={lor} key={currentFolder.indexOf(lor)}/>
				)));
				for (let i = 1; i <= Math.ceil(completedRequests.length / todosPerPage); i++) {
					pageNumbers.push(i);
				}
...
GetAllCompletedRequests.propTypes = {
...
export default connect(mapStateToProps, {getAllCompletedRequests})(GetAllCompletedRequests)
